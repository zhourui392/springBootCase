$.fn.teninedialog = function(options){
    var defaults = {
        ...
        dialogShown:function(modal){},//对话框已经显示事件
    ...
    return this.each(function(){
        var obj=$(this);
        modalID=getModalID();
        var tmpHtml='<div id="{ID}" style="z-index: 2000;" class="modal hide fade" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">' +
            '<div class="modal-header">' +
            '<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>' +
            '<h3 id="myModalLabel" style="{titleStyle}">{title}</h3>' +
            '</div>' +
        ...
        if(!options.showCloseButton){buttonHtml='';}
        ...
        //替换模板标记
        tmpHtml=tmpHtml.replace(/{ID}/g,modalID).replace(/{title}/g,options.title).replace(/{titleStyle}/g,options.titleStyle).replace(/{body}/g,options.content).replace(/{button}/g,buttonHtml);
        obj.append(tmpHtml);
